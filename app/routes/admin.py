...
def staff():
    ...
    if request.args.get('partial'):
        return render_template('admin/staff_partial.html', users=users, search=search, current_user=current_user)
...
def curriculum_subjects():
    ...
    # O'quv rejaga biriktirilgan fanlar
    ...
    if search:
        ...
        # SQL query'da qidirish: fan nomi, tavsif, qabul yili, yo'nalish nomi, yo'nalish kodi, ta'lim shakli
        query = query.filter(
            or_(
                Subject.name.ilike(f'%{search}%'),
                Subject.description.ilike(f'%{search}%'),
            ...
            if matching_groups or matching_teachers:
                ...
                if matching_teachers:
                    ...
                    for group in active_groups:
                        ...
                        if group_teacher_ids & teacher_ids:  # Intersection
                            filtered_groups.append(group)
                    if filtered_groups:
                        active_groups = filtered_groups
                    else:
                        # Agar o'qituvchi bo'yicha topilgan bo'lsa, lekin guruh topilmagan bo'lsa, hali ham ko'rsatish
                        pass
            else:
                # Agar guruh yoki o'qituvchi bo'yicha topilmasa, lekin SQL query'da topilgan bo'lsa, ko'rsatish
                # (fan nomi, yo'nalish nomi, kodi, qabul yili, ta'lim shakli bo'yicha)
                pass
