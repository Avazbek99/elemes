...
from app.models import User, Faculty, Group, Subject, TeacherSubject, TeacherDepartment, Assignment, Direction, GradeScale, Schedule, UserRole, RolePermission, StudentPayment, DirectionCurriculum, Message, Submission, Lesson, LessonView, Announcement, PasswordResetToken, SiteSetting, FlashMessage, Department
from app import db
...
DEFAULT_ROLE_PERMISSIONS = [
    ...
    ('department_head', 'view_admin_panel'), ('department_head', 'view_departments'), ('department_head', 'edit_department'), ('department_head', 'view_subjects'),
...
def create_staff():
    ...
    if request.method == 'POST':
        ...
        # Kafedra mudiri roli tanlangan bo'lsa, kafedrani aniqlash (majburiy)
        if 'department_head' in selected_roles:
            managed_dept_id = request.form.get('managed_department_id', type=int)
            if not managed_dept_id:
                flash(t('department_required_for_department_head'), 'error')
                faculties = Faculty.query.all()
                departments = Department.query.order_by(Department.name).all()
                return render_template('admin/create_staff.html', faculties=faculties, departments=departments)
            dept = Department.query.get(managed_dept_id)
            if not dept:
                flash(t('department_not_found'), 'error')
                faculties = Faculty.query.all()
                departments = Department.query.order_by(Department.name).all()
                return render_template('admin/create_staff.html', faculties=faculties, departments=departments)
...
def edit_staff(id):
    ...
    if request.method == 'POST':
        ...
        if not login:
            ...
            faculties = Faculty.query.all()
            departments = Department.query.order_by(Department.name).all()
            return render_template('admin/edit_staff.html', user=user, existing_roles=existing_roles, faculties=faculties, departments=departments)
...
def departments():
    ...
    # Kafedra mudiri faqat o'z kafedrasini ko'radi
    if current_user.has_role('department_head') and current_user.managed_department_id and not getattr(current_user, 'is_superadmin', False):
        departments_query = departments_query.filter(Department.id == current_user.managed_department_id)
    if search:
        departments_query = departments_query.filter(Department.name.ilike(f'%{search}%'))

    departments_list = departments_query.order_by(Department.name).all()
...
def create_department():
    if request.method == 'POST':
        ...
        if not name:
            flash(t('name_required'), 'error')
            return render_template('admin/create_department.html')
        dept = Department(name=name, description=description)
        db.session.add(dept)
        db.session.commit()
        flash(t('department_created', name=name), 'success')
...
def department_detail(id):
    ...
    if current_user.has_role('department_head') and current_user.managed_department_id and current_user.managed_department_id != dept.id and not getattr(current_user, 'is_superadmin', False):
        flash(t('no_access_permission'), 'error')
        return redirect(url_for('admin.departments'))
...
def edit_department(id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash(t('department_updated', name=name), 'success')
        return redirect(url_for('admin.department_detail', id=dept.id))
    return render_template('admin/edit_department.html', department=dept)
...
def directions():
    ...
    directions_list_data.sort(key=lambda x: ((x['enrollment_year'] or 9999), (x['faculty'].name if x.get('faculty') else ''), x['code'] or '', x['name'] or ''))
    ...
    # O'quv bo'limi uchun jadval ro'yxati (ustunlar: fakultet, qabul yili, yo'nalish kodi nomi, ta'lim shakli, amallar)
    edu_directions_table = []
    for item in courses_list:
        for d in item['directions']:
            faculty_name = (d.get('faculty').name if d.get('faculty') else '') or ''
