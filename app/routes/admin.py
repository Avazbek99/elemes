...
def index():
    stats = {
        'total_users': User.count_excluding_superadmins(),
...
# ==================== TIZIM YANGILANISHI (institutlarda qo'lda tekshirish va yangilash) ====================
def _is_central_server():
    """Markaz serverida ishlayapmizmi. IS_CENTRAL_SERVER=True, CENTRAL_API_URL bo'sh yoki o'zimizga qarasa."""
    if current_app.config.get('IS_CENTRAL_SERVER'):
        return True
    url = (current_app.config.get('CENTRAL_API_URL') or '').strip()
    if not url:
        return True
    try:
        from urllib.parse import urlparse
        central_host = urlparse(url).netloc.split(':')[0].lower()
...
@superadmin_required
def update_page():
    """Yangilanish mavjudligini tekshirish va qo'lda yangilash (faqat superadmin)."""
    from app.services.sse_client import is_enabled
    is_central = _is_central_server()
    institutions = []
    center_blocked = False
    if is_central:
        try:
            from app.central_api import get_institutions_with_status, get_center_block_status
            institutions = get_institutions_with_status()
            center_blocked = get_center_block_status().get('blocked', False)
        except Exception:
            pass
    if not is_enabled(current_app):
        return render_template('admin/update.html', enabled=False, current_version='', latest_version=None, update_available=False, is_central=is_central, institutions=institutions, center_blocked=center_blocked)
    from app.services.updater import get_current_version
    current_version = get_current_version()
    return render_template('admin/update.html', enabled=True, current_version=current_version, latest_version=None, update_available=False, is_central=is_central, institutions=institutions, center_blocked=center_blocked)
...
@superadmin_required
def update_check():
    """So'nggi versiyani tekshirish (JSON). Markazda version.json, institutda markaz API."""
    from app.services.sse_client import is_enabled
    if not is_enabled(current_app):
        return jsonify({'ok': False, 'error': t('update_center_no_connection'), 'current_version': ''}), 400
    ...
    if not v or v.get('error_key') or v.get('error'):
        err_msg = None
        if isinstance(v, dict) and v.get('error_key'):
            key = v.get('error_key')
            url = v.get('error_url') or ''
            code = v.get('error_code')
            if key == 'update_center_bad_response':
                err_msg = t(key, code=code or '', url=url)
            elif key in ('update_center_timeout', 'update_center_connection_error'):
                err_msg = t(key, url=url)
            else:
                err_msg = t(key)
        if not err_msg:
            err_msg = t('update_version_failed')
        return jsonify({'ok': False, 'error': err_msg, 'current_version': current}), 200
    latest = (v.get('version') or '').strip()
    if not latest:
        return jsonify({'ok': False, 'error': t('update_version_failed'), 'current_version': current}), 200
    update_available = is_central_newer(latest, current)
    return jsonify({'ok': True, 'current_version': current, 'latest_version': latest, 'update_available': update_available})
...
@superadmin_required
def update_run():
    """Yangilanishni qo'lda o'rnatish (vaqt oynasiga qaramay)."""
    from app.services.sse_client import is_enabled
    if not is_enabled(current_app):
        flash(t('no_access_permission'), 'error')
    ...
    if run_update():
        flash(t('update_installed_restart') or 'Yangilanish o\'rnatildi. Tizim tez orada qayta ishga tushadi.', 'success')
        schedule_restart()
    else:
        flash(t('update_not_applied') or 'Yangilanish o\'tkazilmadi (so\'nggi versiya o\'rnatilgan yoki xato).', 'warning')
    return redirect(url_for('admin.update_page'))


@bp.route('/update/center-block', methods=['POST'])
@login_required
@superadmin_required
def update_center_block_set():
    """Markazni bloklash yoki blokni olish (faqat markazda). Institutlar ishlashda qoladi."""
    if not _is_central_server():
        flash(t('no_access_permission'), 'error')
        return redirect(url_for('admin.update_page'))
    try:
        from app.central_api import set_center_block
        blocked = request.form.get('blocked') == '1'
        reason = (request.form.get('block_reason') or '').strip()
        set_center_block(blocked, reason)
        if blocked:
            flash(t('center_blocked') or "Markaz bloklandi. Institutlar ishlashda davom etadi.", 'warning')
        else:
            flash(t('center_not_blocked') or "Markaz bloki olindi.", 'success')
    except Exception as e:
        flash(str(e), 'error')
    return redirect(url_for('admin.update_page'))


@bp.route('/update/trigger', methods=['POST'])
@login_required
@superadmin_required
def update_trigger():
    """Barcha SSE ulangan institutlarga yangilanish xabarini yuborish (faqat markazda)."""
    if not _is_central_server():
        flash(t('no_access_permission'), 'error')
        return redirect(url_for('admin.update_page'))
    try:
        from app.central_api import trigger_update_now
        trigger_update_now()
        flash(t('update_broadcast_sent') or "Yangilanish xabari barcha institutlarga yuborildi.", 'success')
    except Exception as e:
        flash((t('update_broadcast_error') or 'Xabar yuborishda xato: ') + str(e), 'error')
    return redirect(url_for('admin.update_page'))


@bp.route('/update/institutions', methods=['GET'])
@login_required
@superadmin_required
def update_institutions_list_route():
    """Institutlar ro'yxati (JSON) – faqat markazda."""
    if not _is_central_server():
        return jsonify({'error': 'Forbidden'}), 403
    try:
        from app.central_api import get_institutions_with_status
        return jsonify(get_institutions_with_status())
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@bp.route('/update/institutions', methods=['POST'])
@login_required
@superadmin_required
def update_institutions_save():
    """Institutlar ro'yxatini yangilash (bitta yoki bir nechta) – faqat markazda. Form yoki JSON qabul qiladi."""
    if not _is_central_server():
        ...
        return redirect(url_for('admin.update_page'))
    try:
        if request.form and request.form.get('id'):
            # Form orqali (qo'shish/tahrirlash) – CSRF avtomatik
            inst_id = request.form.get('id', '').strip()
            if request.form.get('mode') == 'add':
                from app.central_api import get_institutions_with_status
                existing = get_institutions_with_status()
                if any(str(i.get('id')) == inst_id for i in existing):
                    flash(t('institution_id_exists'), 'error')
                    return redirect(url_for('admin.update_page'))
            updated = [{
                'id': inst_id,
                'name': request.form.get('name', '').strip() or inst_id,
                'description': (request.form.get('description') or '').strip(),
                'blocked': request.form.get('blocked') == '1',
                'block_reason': (request.form.get('block_reason') or '').strip()
            }]
        else:
            payload = request.get_json(silent=True) or {}
            updated = payload.get('institutions', [payload]) if isinstance(payload.get('institutions'), list) else [payload]
            if not updated and isinstance(payload, list):
                updated = payload
        if not updated or not str(updated[0].get('id', '')).strip():
            flash(t('institution_id_label') or "Institut ID", 'warning')
            return redirect(url_for('admin.update_page'))
        from app.central_api import update_institutions_list
        institutions = update_institutions_list(updated)
        if request.content_type and 'application/json' in request.content_type:
            return jsonify({'status': 'success', 'institutions': institutions})
        flash(t('institutions_updated') or "Institutlar ro'yxati yangilandi.", 'success')
        return redirect(url_for('admin.update_page'))
    except Exception as e:
        if request.content_type and 'application/json' in request.content_type:
...
@bp.route('/update/institutions/<inst_id>/toggle', methods=['POST'])
@login_required
@superadmin_required
def update_institution_toggle(inst_id):
    """Institut faolligini o'zgartirish (bloklash/blokni olish) – faqat markazda."""
    if not _is_central_server():
        flash(t('no_access_permission'), 'error')
        return redirect(url_for('admin.update_page'))
    try:
        from app.central_api import get_institutions_with_status, update_institutions_list
        institutions = get_institutions_with_status()
        inst = next((i for i in institutions if str(i.get('id')) == str(inst_id)), None)
        if not inst:
            flash(t('no_data') or "Institut topilmadi.", 'warning')
            return redirect(url_for('admin.update_page'))
        blocked = not bool(inst.get('blocked', False))  # flip
        update_institutions_list([{'id': str(inst_id), 'name': inst.get('name'), 'blocked': blocked, 'block_reason': inst.get('block_reason', '') if blocked else ''}])
        flash(t('institutions_updated') or "Institut yangilandi.", 'success')
    except Exception as e:
        flash(str(e), 'error')
    return redirect(url_for('admin.update_page'))


@bp.route('/update/institutions/<inst_id>/delete', methods=['POST'])
@login_required
@superadmin_required
def update_institution_delete(inst_id):
    """Institutni ro'yxatdan o'chirish – faqat markazda."""
    if not _is_central_server():
        flash(t('no_access_permission'), 'error')
        return redirect(url_for('admin.update_page'))
    try:
        from app.central_api import delete_institution
        delete_institution(inst_id)
        flash(t('institution_deleted') or "Institut ro'yxatdan o'chirildi.", 'success')
    except Exception as e:
        flash(str(e), 'error')
    return redirect(url_for('admin.update_page'))
...
def delete_superadmin(id):
    ...
    if len(super_list) <= 1:
        ...
        return redirect(url_for('admin.superadmins'))
    # Bog'liq jadvallarni tozalash (delete_user bilan bir xil)
    ...
    Schedule.query.filter_by(teacher_id=user.id).update({Schedule.teacher_id: None}, synchronize_session=False)
    StudentPayment.query.filter_by(student_id=user.id).delete(synchronize_session=False)
    StaffAttendanceDaily.query.filter_by(staff_id=user.id).delete(synchronize_session=False)
    StudentAttendanceDaily.query.filter_by(student_id=user.id).delete(synchronize_session=False)
    Lesson.query.filter_by(created_by=user.id).update({Lesson.created_by: None}, synchronize_session=False)
    Assignment.query.filter_by(created_by=user.id).update({Assignment.created_by: None}, synchronize_session=False)
    Submission.query.filter_by(graded_by=user.id).update({Submission.graded_by: None}, synchronize_session=False)
    Test.query.filter_by(created_by=user.id).update({Test.created_by: None}, synchronize_session=False)
...
@superadmin_required
def face_logs():
    """Hikvision yuz tanilash loglari – faqat superadmin. 50 ta qator, pagination xodimlar bazasidagi kabi."""
    per_page = 50
    page = max(1, request.args.get('page', 1, type=int))
    last_request_info = None
    # Sana filtri: default bugun (Toshkent)
    tz = timedelta(hours=5)
    today_tashkent = (datetime.utcnow() + tz).date()
    date_from_s = request.args.get('date_from', '')
    date_to_s = request.args.get('date_to', '')
    try:
        date_from = date.fromisoformat(date_from_s) if date_from_s else today_tashkent
        date_to = date.fromisoformat(date_to_s) if date_to_s else today_tashkent
    except ValueError:
    ...
    pagination = None
    try:
        from app.face_api.routes import _is_valid_person_name, _dedupe_face_logs, _staff_full_names_for_logs
        ...
        if search_query:
            q = q.outerjoin(User, User.employee_code == FaceLog.device_employee_id).filter(
                or_(
                    FaceLog.person_name.ilike('%' + search_query + '%'),
                    FaceLog.device_employee_id.ilike('%' + search_query + '%'),
                    User.full_name.ilike('%' + search_query + '%'),
                )
            )
        q = q.order_by(order_fn)
        full_list = q.limit(5000).all()
        full_list = [log for log in full_list if _is_valid_person_name((log.person_name or '').strip())]
        full_list = _dedupe_face_logs(full_list)
        total = len(full_list)
        start_idx = (page - 1) * per_page
        end_idx = start_idx + per_page
        logs_list = full_list[start_idx:end_idx]
        staff_names = _staff_full_names_for_logs(logs_list)
        for log in logs_list:
            fid = (log.device_employee_id or '').strip()
            log.staff_full_name = staff_names.get(fid) or None
        pagination = Pagination(logs_list, page, per_page, total)
    except Exception as e:
        try:
            db.create_all()
            from app.face_api.routes import _is_valid_person_name, _dedupe_face_logs, _staff_full_names_for_logs
    ...
    try:
        ...
        pass
    return render_template('admin/face_logs.html',
        logs_list=logs_list, pagination=pagination, last_request_info=last_request_info,
...
def delete_user(id):
    ...
    if user.id == current_user.id:
        ...
        TeacherDepartment.query.filter_by(teacher_id=user.id).delete(synchronize_session=False)
        # UserFaculty (dekan fakultetlari)
        UserFaculty.query.filter_by(user_id=user.id).delete(synchronize_session=False)
        # Schedule teacher_id ni NULL qilish
        Schedule.query.filter_by(teacher_id=user.id).update({Schedule.teacher_id: None}, synchronize_session=False)
        # StudentPayment (talaba to'lovlari)
        StudentPayment.query.filter_by(student_id=user.id).delete(synchronize_session=False)
        # Davomat (StaffAttendanceDaily, StudentAttendanceDaily)
        StaffAttendanceDaily.query.filter_by(staff_id=user.id).delete(synchronize_session=False)
        StudentAttendanceDaily.query.filter_by(student_id=user.id).delete(synchronize_session=False)
        # created_by, graded_by NULL qilish (Lesson, Assignment, Submission, Test)
        Lesson.query.filter_by(created_by=user.id).update({Lesson.created_by: None}, synchronize_session=False)
        Assignment.query.filter_by(created_by=user.id).update({Assignment.created_by: None}, synchronize_session=False)
        Submission.query.filter_by(graded_by=user.id).update({Submission.graded_by: None}, synchronize_session=False)
        Test.query.filter_by(created_by=user.id).update({Test.created_by: None}, synchronize_session=False)
        ...
        if 'staff' in ref:
            flash(t('staff_deleted', staff_name=user.full_name or 'Xodim'), 'success')
        elif 'students' in ref:
            flash(t('student_deleted', student_name=user.full_name or 'Talaba'), 'success')
        else:
...
def create_staff():
    ...
    if request.method == 'POST':
        ...
        device_employee_id = (request.form.get('device_employee_id') or '').strip() or None
        
        # Bir nechta rol tanlash (ixtiyoriy; tanlanmasa "Xodim")
        selected_roles = request.form.getlist('roles')  # ['admin', 'dean', 'teacher', 'xodim']
        selected_roles = [r for r in selected_roles if r != 'xodim']  # boshqa rol bor bo'lsa faqat ular
        if not selected_roles:
            selected_roles = ['xodim']
        ...
        if 'admin' in selected_roles:
            ...
            main_role = 'teacher'
        elif 'accounting' in selected_roles:
            main_role = 'accounting'
        # xodim — qolgan hollar uchun (rol tanlanmaganida ham)
        # Bazada User.role faqat ma'lum qiymatlarni qabul qilsa, User jadvalida 'teacher' saqlaymiz; ko'rinish uchun UserRole da 'xodim' qoladi
        db_role = main_role if main_role != 'xodim' else 'teacher'
        ...
        try:
            user = User(
                login=login,
                full_name=full_name,
                role=db_role,  # Bazada qabul qilinadigan rol; ko'rinish UserRole orqali (xodim saqlanadi)
                ...
                employee_code=device_employee_id  # F.ID — User modelida employee_code
            )
            ...
            faculties = Faculty.query.all()
            departments = Department.query.order_by(Department.name).all()
            flash(t('staff_create_error') or ('Xatolik: ' + str(e)), 'error')
            return render_template('admin/create_staff.html', faculties=faculties, departments=departments)
...
def edit_staff(id):
    ...
    if request.method == 'POST':
        ...
        user.employee_code = (request.form.get('device_employee_id') or '').strip() or None  # F.ID
        
        # Bir nechta rol tanlash (ixtiyoriy; tanlanmasa "Xodim")
        selected_roles = request.form.getlist('roles')
        selected_roles = [r for r in selected_roles if r != 'xodim']
        if not selected_roles:
            selected_roles = ['xodim']
        ...
        if 'admin' in selected_roles:
            ...
            main_role = 'accounting'
        # xodim — qolgan hollar uchun; bazada User.role uchun 'teacher' saqlanadi, ko'rinish UserRole orqali
        db_role = main_role if main_role != 'xodim' else 'teacher'
        user.role = db_role
...
def delete_department(id):
    ...
    if current_user.has_role('department_head') and not current_user.has_role('admin') and not getattr(current_user, 'is_superadmin', False):
        if not DepartmentHead.query.filter_by(department_id=dept.id, user_id=current_user.id).first() and current_user.managed_department_id != dept.id:
            ...
            return redirect(url_for('admin.departments'))
    dept_name = dept.name
    head_user_ids = [link.user_id for link in DepartmentHead.query.filter_by(department_id=id).all()]
    ...
    for uid in head_user_ids:
        if DepartmentHead.query.filter_by(user_id=uid).count() == 0:
            UserRole.query.filter_by(user_id=uid, role='department_head').delete(synchronize_session=False)
...
@bp.route('/departments/clear_all', methods=['POST'])
@login_required
@superadmin_required
def clear_all_departments():
    """Barcha kafedralarni tozalash. Faqat superadmin. Kafedra mudiri roli barcha xodimlardan olib tashlanadi."""
    departments = Department.query.all()
    count = 0
    for dept in departments:
        Subject.query.filter_by(department_id=dept.id).update({'department_id': None}, synchronize_session=False)
        SubjectDepartment.query.filter_by(department_id=dept.id).delete(synchronize_session=False)
        TeacherDepartment.query.filter_by(department_id=dept.id).delete(synchronize_session=False)
        DepartmentHead.query.filter_by(department_id=dept.id).delete(synchronize_session=False)
        User.query.filter_by(managed_department_id=dept.id).update({'managed_department_id': None}, synchronize_session=False)
        db.session.delete(dept)
        count += 1
    UserRole.query.filter_by(role='department_head').delete(synchronize_session=False)
    db.session.commit()
    flash(t('departments_cleared', count=count), 'success')
    return redirect(url_for('admin.departments'))
...
@bp.route('/subjects/clear_all', methods=['POST'])
@login_required
@superadmin_required
def clear_all_subjects():
    """Barcha fanlarni tozalash (bog'liq reja, dars, topshiriq, jadval, testlar ham). Faqat superadmin."""
    from app.models import TestSubmission, TestQuestion, TestOption
    subjects = Subject.query.all()
    count = 0
    for subject in subjects:
        sid = subject.id
        DirectionCurriculum.query.filter_by(subject_id=sid).delete(synchronize_session=False)
        lesson_ids = [l.id for l in Lesson.query.filter_by(subject_id=sid).all()]
        if lesson_ids:
            LessonView.query.filter(LessonView.lesson_id.in_(lesson_ids)).delete(synchronize_session=False)
        Lesson.query.filter_by(subject_id=sid).delete(synchronize_session=False)
        assignment_ids = [a.id for a in Assignment.query.filter_by(subject_id=sid).all()]
        if assignment_ids:
            Submission.query.filter(Submission.assignment_id.in_(assignment_ids)).delete(synchronize_session=False)
        Assignment.query.filter_by(subject_id=sid).delete(synchronize_session=False)
        Schedule.query.filter_by(subject_id=sid).delete(synchronize_session=False)
        test_ids = [t.id for t in Test.query.filter_by(subject_id=sid).all()]
        if test_ids:
            question_ids = [q.id for q in TestQuestion.query.filter(TestQuestion.test_id.in_(test_ids)).all()]
            if question_ids:
                TestOption.query.filter(TestOption.question_id.in_(question_ids)).delete(synchronize_session=False)
            TestQuestion.query.filter(TestQuestion.test_id.in_(test_ids)).delete(synchronize_session=False)
            TestSubmission.query.filter(TestSubmission.test_id.in_(test_ids)).delete(synchronize_session=False)
        Test.query.filter_by(subject_id=sid).delete(synchronize_session=False)
        TeacherSubject.query.filter_by(subject_id=sid).delete(synchronize_session=False)
        SubjectDepartment.query.filter_by(subject_id=sid).delete(synchronize_session=False)
        db.session.delete(subject)
        count += 1
    db.session.commit()
    flash(t('subjects_cleared', count=count), 'success')
    return redirect(url_for('admin.subjects'))
...
@bp.route('/staff/clear_all', methods=['POST'])
@login_required
@superadmin_required
def clear_all_staff():
    """Barcha xodimlarni tozalash (superadmin va joriy foydalanuvchi saqlanadi)"""
    staff_roles = ['admin', 'dean', 'edu_dept', 'department_head', 'teacher', 'accounting']
    staff_user_ids = set()
    ...
    for user in staff_users:
        ...
        count += 1
    db.session.commit()
    flash(t('staff_cleared', count=count), 'success')
    return redirect(url_for('admin.staff'))


@bp.route('/students/clear_all', methods=['POST'])
@login_required
@superadmin_required
def clear_all_students():
    """Barcha talabalarni tozalash (superadmin va joriy foydalanuvchi saqlanadi). Faqat superadmin."""
    from app.models import TestSubmission
    students = User.query.filter(User.role == 'student').all()
    students = [u for u in students if not getattr(u, 'is_superadmin', False) and u.id != current_user.id]
    count = 0
    for student in students:
        ...
        StudentPayment.query.filter_by(student_id=student.id).delete(synchronize_session=False)
        StudentAttendanceDaily.query.filter_by(student_id=student.id).delete(synchronize_session=False)
        TestSubmission.query.filter_by(student_id=student.id).delete(synchronize_session=False)
        Announcement.query.filter_by(author_id=student.id).delete(synchronize_session=False)
        UserRole.query.filter_by(user_id=student.id).delete(synchronize_session=False)
        db.session.delete(student)
        count += 1
    db.session.commit()
    flash(t('students_cleared', count=count), 'success')
    return redirect(url_for('admin.students'))
...
def create_student():
    ...
    if request.method == 'POST':
        ...
        student = User(
            ...
            description=description.strip() if description and description.strip() else None,
            enrollment_year=enrollment_year,
            employee_code=device_employee_id
        )
...
def edit_student(id):
    ...
    if request.method == 'POST':
        ...
        student.employee_code = (request.form.get('device_employee_id') or '').strip() or None
...
def delete_student(id):
    ...
    # Davomat (student_id NOT NULL – o'chirish kerak)
    StudentAttendanceDaily.query.filter_by(student_id=student.id).delete(synchronize_session=False)
    ...
    # E'lonlar (agar talaba yozgan bo'lsa)
    Announcement.query.filter_by(author_id=student.id).delete(synchronize_session=False)
