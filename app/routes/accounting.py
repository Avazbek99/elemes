...
@bp.route('/contract-amounts/clear_all', methods=['POST'])
@login_required
def clear_all_contract_amounts():
    """Barcha to'lov summalarini (kontrakt summalarini) tozalash. Faqat superadmin."""
    if not getattr(current_user, 'is_superadmin', False):
        flash(t('no_access_permission'), 'error')
        return redirect(url_for('accounting.contract_amounts'))
    count = DirectionContractAmount.query.delete()
    db.session.commit()
    flash(t('contract_amounts_cleared', count=count), 'success')
    return redirect(url_for('accounting.contract_amounts'))
