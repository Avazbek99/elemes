...
@bp.route('/<int:id>/<int:dir_id>/<int:group_id>', methods=['GET', 'POST'])
@bp.route('/<int:id>/<int:dir_id>', methods=['GET', 'POST'], defaults={'group_id': None})
@bp.route('/<int:id>', methods=['GET', 'POST'], defaults={'dir_id': None, 'group_id': None})
@login_required
def detail(id, dir_id=None, group_id=None):
    """Fan tafsilotlari (URL: /subjects/21/1/5 yoki /subjects/21/1/5/M/1)"""
        ...
        if not gr or gr.direction_id != direction_id:
            ...
            if not ts_check:
                ...
                if ts_check and ts_check.group_id != group_id:
                    return redirect(url_for('courses.detail', id=id, dir_id=direction_id, group_id=ts_check.group_id))
                elif not ts_check:
                    group_id = None
    ...
    if direction_id and group_id is None:
        ...
        if target_group_id:
            return redirect(url_for('courses.detail', id=id, dir_id=direction_id, group_id=target_group_id))
    ...
    for ta in all_teacher_assignments:
        ...
        if should_add_lesson_type:
            lt_normalized = (ta.lesson_type or '').strip().lower()
            # Yo'nalish tanlangan va o'quv reja bor bo'lsa, faqat soat kiritilgan dars turlarini ko'rsatish
            if target_direction_id and curriculum_item and lt_normalized not in allowed_lesson_types:
...
@login_required
def hierarchical_lesson_detail(subject_id, direction_id, group_id, type_code, order):
    """Ierarxik dars tafsilotlari - URL /subjects/21/1/5/M/2 formatida"""
    type_map = {
        'M': 'maruza',
        'A': 'amaliyot',
        'L': 'laboratoriya',
    ...
    # Guruh va yo'nalish bo'yicha darsni qidirish: (group_id mos yoki None) va (direction_id mos yoki None)
    lesson = Lesson.query.filter(
        ...
        (Lesson.group_id == group_id) | (Lesson.group_id.is_(None)),
        (Lesson.direction_id == direction_id) | (Lesson.direction_id.is_(None))
    ).order_by(Lesson.group_id.desc().nullslast(), Lesson.direction_id.desc().nullslast()).first_or_404()
    
    # lesson_detail bilan bir xil template - to'g'ridan-to'g'ri render (redirect emas)
...
def create_assignment(id):
    ...
    if is_acting_as_teacher:
        ...
        if (current_user.role == 'admin' or current_user.role == 'dean') and current_role != 'teacher' and direction_id:
            curriculum_item = DirectionCurriculum.query.filter_by(
                direction_id=direction_id,
                subject_id=subject.id
            ).first()
...
def edit_assignment(id):
    ...
    elif (current_user.role == 'admin' or current_user.role == 'dean') and current_role != 'teacher' and direction_id:
        curriculum_item = DirectionCurriculum.query.filter_by(
            direction_id=direction_id,
            subject_id=subject.id
        ).first()
