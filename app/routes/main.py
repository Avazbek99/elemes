...
@login_required
def live_monitor():
    """Davomat live monitor – Hikvision log real vaqtda (superadmin)."""
    if not getattr(current_user, 'is_superadmin', False):
        abort(403)
    return render_template('attendance/live_monitor.html')


@bp.route('/smart-dashboard')
def smart_dashboard():
    """
    Smart Attendance Dashboard – TV rejimida (?tv=1) login talab qilinmaydi,
    oddiy kirishda superadmin kerak.
    """
    ...
    if not tv_mode:
        ...
        if not getattr(current_user, 'is_superadmin', False):
            abort(403)
    return render_template('attendance/smart_dashboard.html', tv_mode=tv_mode)
...
def _dashboard_inner():
    ...
    from sqlalchemy import func as _sql_func  # explicit import to avoid UnboundLocalError
    user = current_user
    ...
    if active_role == 'student':
        ...
        if user.group_id:
            ...
            if all_subj_ids:
                ...
                if total_semester_max_score > 0:
                    semester_progress = (total_semester_score / total_semester_max_score) * scale_max
                    semester_grade = GradeScale.get_grade(semester_progress, scale_max)
                else:
                    semester_progress = 0
                    semester_grade = None
        ...
        # Admin uchun (superadminlar jami foydalanuvchilar soniga kirmaydi)
        stats = {
    ...
    if active_role in ('admin', 'accounting', 'dean'):
        ...
        payment_total_paid = db.session.query(_sql_func.sum(StudentPayment.paid_amount)).filter(
    ...
    if active_role in ('admin', 'accounting', 'dean'):
        ...
        payment_total_paid = float(db.session.query(_sql_func.sum(StudentPayment.paid_amount)).filter(
    ...
    try:
        return render_template('dashboard.html', stats=stats, **stats, announcements=announcements,
                             ...
                             semester_scale_max=scale_max if 'scale_max' in locals() else 100,
...
def chat(user_id):
    ...
    # Ikki foydalanuvchi o'rtasidagi barcha xabarlar (qilinguvchi yuqorida, keyin vaqt)
    ...
    # Forward modal uchun boshqa foydalanuvchilar (messages sahifasidagi kabi)
    available_users = []
    if user.role == 'student':
        if user.group_id:
            faculty_id = user.group.faculty_id if user.group else None
            teacher_ids = [ts.teacher_id for ts in TeacherSubject.query.filter_by(group_id=user.group_id).all()]
            filters = [User.group_id == user.group_id, User.id.in_(teacher_ids), User.role.in_(['admin', 'accounting'])]
            if faculty_id:
                filters.append((User.role == 'dean') & (User.faculty_id == faculty_id))
            available_users = User.query.filter(User.id != user.id, User.is_active == True, or_(*filters)).all()
        else:
            available_users = User.query.filter(User.id != user.id, User.is_active == True, User.role.in_(['admin', 'accounting'])).all()
    elif user.role == 'teacher':
        group_ids = [ts.group_id for ts in TeacherSubject.query.filter_by(teacher_id=user.id).all()]
        available_users = User.query.filter(User.id != user.id, User.is_active == True, or_(User.group_id.in_(group_ids or [0]), User.role.in_(['teacher', 'dean', 'admin', 'accounting']))).all()
    else:
        available_users = User.query.filter(User.id != user.id, User.is_active == True).all()
    available_users = [u for u in available_users if not getattr(u, 'is_superadmin', False) and u.id != other_user.id]
    
    return render_template('chat.html', other_user=other_user, messages=messages, available_users=available_users)
...
def chat_messages_api(user_id):
    ...
    for m in messages:
        ...
        if getattr(m, 'reply_to_id', None) and m.reply_to:
            item['reply_to_content'] = (m.reply_to.content or '')[:80]
        out.append(item)
    return jsonify({'messages': out})


@bp.route('/api/chat/<int:user_id>/messages/<int:message_id>', methods=['PATCH', 'DELETE'])
@login_required
def chat_message_edit_delete_api(user_id, message_id):
    """Xabarni tahrirlash (PATCH) yoki o'chirish (DELETE)."""
    msg = Message.query.filter_by(id=message_id).first()
    if not msg:
        return jsonify({'error': 'not_found'}), 404
    in_chat = (msg.sender_id == current_user.id and msg.receiver_id == user_id) or (msg.sender_id == user_id and msg.receiver_id == current_user.id)
    if not in_chat:
    ...
    # PATCH – faqat content tahrirlash
...
@bp.route('/api/chat/forward', methods=['POST'])
@login_required
def chat_forward_api():
    """Xabarni boshqa foydalanuvchiga forward qilish."""
    data = request.get_json(silent=True) or {}
    message_id = data.get('message_id', type=int)
    to_user_id = data.get('to_user_id', type=int)
    if not message_id or not to_user_id:
        return jsonify({'error': 'message_id and to_user_id required'}), 400
    msg = Message.query.get(message_id)
    if not msg:
        return jsonify({'error': 'not_found'}), 404
    in_chat = (msg.sender_id == current_user.id) or (msg.receiver_id == current_user.id)
    if not in_chat:
        return jsonify({'error': 'forbidden'}), 403
    ...
    if to_user.id == current_user.id:
        return jsonify({'error': 'cannot forward to self'}), 400
    # Ruxsat: to_user bilan suhbatlashish imkoniyati (chat() dagi allowed logika qisqacha)
    allowed = False
    if current_user.role in ['admin', 'dean']:
        allowed = True
    elif current_user.role == 'student' and current_user.group_id:
        if to_user.role == 'student' and to_user.group_id == current_user.group_id:
            allowed = True
        elif to_user.role == 'teacher' and TeacherSubject.query.filter_by(teacher_id=to_user.id, group_id=current_user.group_id).first():
            allowed = True
        elif to_user.role == 'dean' and current_user.group and current_user.group.faculty_id == to_user.faculty_id:
            allowed = True
        elif to_user.role in ['admin', 'accounting']:
            allowed = True
    elif current_user.role == 'teacher':
        if to_user.role == 'student' and TeacherSubject.query.filter_by(teacher_id=current_user.id, group_id=to_user.group_id).first():
            allowed = True
        elif to_user.role in ['teacher', 'dean', 'admin', 'accounting']:
            allowed = True
    if not allowed:
        return jsonify({'error': 'forbidden'}), 403
    forwarded = Message(
        ...
        content=('[Forwarded] ' + (msg.content or ''))[:2000]
    )
    db.session.add(forwarded)
    db.session.commit()
    return jsonify({'ok': True, 'id': forwarded.id})


@bp.route('/api/chat/translate', methods=['POST'])
@login_required
def chat_translate_api():
    """Xabar matnini boshqa tilga tarjima qilish (deep_translator)."""
    data = request.get_json(silent=True) or {}
    text = (data.get('text') or '').strip()
    target_lang = (data.get('target_lang') or 'uz').strip().lower()
    if not text:
    ...
    try:
        ...
        # GoogleTranslator da source='auto' va target='uz'|'ru'|'en'
        translated = GoogleTranslator(source='auto', target=target_lang).translate(text[:5000]) or text
        return jsonify({'translated': translated, 'target_lang': target_lang})
    except Exception as e:
        return jsonify({'error': 'translate_failed', 'message': str(e)}), 500
