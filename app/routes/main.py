...
def _build_edu_dept_teachers_list():
    ...
    from app.models import DirectionCurriculum, Lesson, TeacherDepartment, Department
    teacher_dept_cache = {}
    ...
    for ts in TeacherSubject.query.join(Group, TeacherSubject.group_id == Group.id).filter(Group.students.any()).order_by(TeacherSubject.teacher_id, TeacherSubject.subject_id).all():
        ...
        if teacher.id not in teacher_dept_cache:
            depts = Department.query.join(TeacherDepartment, Department.id == TeacherDepartment.department_id).filter(
                TeacherDepartment.teacher_id == teacher.id
            ).order_by(Department.name).all()
            teacher_dept_cache[teacher.id] = ', '.join(d.name for d in depts) if depts else ''
        result.append({
            'teacher_id': teacher.id,
            'teacher_name': teacher.full_name or teacher.login or '',
            'department_names': teacher_dept_cache[teacher.id],
...
def fan_resurslari():
    ...
    if active_role not in ('edu_dept', 'admin', 'dean') and not current_user.is_superadmin:
