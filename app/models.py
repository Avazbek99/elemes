...
class Subject(db.Model):
    ...
    def get_departments(self):
        ...
        result = []
        for link in self.department_links.all():
            dept = getattr(link, 'department', None)
            if dept:
                result.append(dept)
        return result
...
class User(UserMixin, db.Model):
    ...
    # Hikvision qurilma bilan bog'lash (device_employee_id)
    ...
    @classmethod
    def count_excluding_superadmins(cls):
        """Foydalanuvchilar soni – superadminlar hisobga olinmasdan (dashboard va hisobotlar uchun)."""
        from sqlalchemy import or_
        try:
            from flask import current_app
            ...
            super_login = ''
        q = cls.query.filter(or_(cls.superadmin_flag == None, cls.superadmin_flag == False))
        if super_login:
            q = q.filter(or_(cls.login == None, cls.login != super_login))
        return q.count()
    ...
    def get_role_display(self):
        """Asosiy rol nomini olish (superadmin uchun Superadmin). Rol belgilanmagan xodimlar uchun UserRole dagi 'xodim' asosida 'Xodim' ko'rsatiladi."""
        ...
        user_roles = self.get_roles()
        if user_roles:
            sorted_r = self.get_sorted_roles()
            main = sorted_r[0] if sorted_r else self.role
            return roles.get(main, main) if main else "Xodim"
        return roles.get(self.role, self.role) if self.role else "Xodim"
    ...
    def has_permission(self, permission, for_role=None):
        ...
        defaults = {
            ...
            'accounting': ['view_accounting', 'view_students', 'view_reports', 'manage_payments', 'manage_contracts', 'view_contract_amounts', 'import_payments',
                          'view_announcements', 'send_message', 'view_messages'],
            'xodim': ['view_subjects', 'view_students', 'create_lesson', 'edit_lesson', 'delete_lesson',
                      'create_assignment', 'edit_assignment', 'delete_assignment',
                      'grade_students', 'view_submissions', 'create_announcement',
                      'view_announcements', 'send_message', 'view_messages'],
        }
...
class FaceLog(db.Model):
    ...
    direction = db.Column(db.String(10), nullable=True, default='IN', index=True)  # IN / OUT
    ...
    def _event_time_from_raw(self):
        """Raw dan to'g'ridan to'g'ri dateTime ni o'qiydi — qurilma yuborgan vaqt ko'rsatiladi (JSON yoki multipart)."""
        if not self.raw_data:
            return None
        import re
        raw = self.raw_data if isinstance(self.raw_data, str) else str(self.raw_data)
        # Multipart yoki JSON ichida "dateTime":"2026-02-25T09:04:14+05:00" qidirish
        m = re.search(r'"dateTime"\s*:\s*"([^"]+)"', raw) or re.search(r'"date_time"\s*:\s*"([^"]+)"', raw)
        if m:
            s = m.group(1).strip()
            if len(s) >= 16:
                part = s.split('+')[0].split('Z')[0].strip()[:19]
                try:
                    if 'T' in part:
        ...
        try:
            ...
            if isinstance(data, dict):
                for key in ('dateTime', 'date_time', 'eventTime', 'event_time', 'EventTime', 'DateTime'):
                    val = data.get(key)
                    if not val:
                        continue
                    s = str(val).strip()
    ...
    def display_event_time(self):
        """Ko'rsatish uchun: avval raw dagi dateTime, yo'q bo'lsa DB dagi event_time."""
        from_raw = self._event_time_from_raw()
        if from_raw is not None:
            return from_raw
        return self.event_time

    def get_device_local_ip(self):
        """Raw_data (JSON) dan qurilmaning lokal IP si (ipAddress) ni ajratib oladi. Tashqi IP emas, raw ichidagi ipAddress."""
        import json
        import re
        raw = self.raw_data
        if not raw or not isinstance(raw, str):
            return None
        raw = raw.strip()
        # Multipart bo'lsa event_log ichidagi JSON ni ajratib olish
        if 'event_log' in raw and '{' in raw:
            idx = raw.find('name="event_log"') if 'name="event_log"' in raw else raw.find("name='event_log'")
            if idx >= 0:
                rest = raw[idx:]
                start = rest.find('{')
                if start >= 0:
        ...
        try:
            if raw.startswith('{'):
                data = json.loads(raw)
                if isinstance(data, dict):
                    v = (data.get('ipAddress') or data.get('ip_address') or '').strip()
                    if v:
                        return v
        except Exception:
            pass
        # Regex orqali "ipAddress": "10.10.11.183" qidirish (multipart yoki boshqa format)
        m = re.search(r'"ipAddress"\s*:\s*"([^"]+)"', raw)
        if m:
            return m.group(1).strip() or None
        m = re.search(r'"ip_address"\s*:\s*"([^"]+)"', raw)
        if m:
            return m.group(1).strip() or None
        return None

    def get_device_name(self):
        """Raw_data (JSON) dan qurilma nomini (deviceName) ajratib oladi."""
        import json
        import re
        raw = self.raw_data
        if not raw or not isinstance(raw, str):
        ...
        try:
            if raw.strip().startswith('{'):
                ...
                if isinstance(data, dict):
                    v = (data.get('deviceName') or data.get('device_name') or '').strip()
                    if v:
                        return v
        except Exception:
            pass
        m = re.search(r'"deviceName"\s*:\s*"([^"]+)"', raw)
        if m:
            return m.group(1).strip() or None
        m = re.search(r'"device_name"\s*:\s*"([^"]+)"', raw)
        if m:
            return m.group(1).strip() or None
        return None
    ...
    def to_dict(self):
        ...
        # Ko'rsatish: yangi = Toshkent as-is, eski = +10 soat (ikki marta -5 ni tuzatish)
        disp = self.display_event_time()
        def event_time_iso(dt):
            if dt is None:
                return None
            if isinstance(dt, str):
                return dt
        ...
        return {
            ...
            'event_time': event_time_iso(disp) if disp else None,
            'device_ip': self.device_ip,
            'device_local_ip': self.get_device_local_ip(),
            'device_name': self.get_device_name(),
            'picture_path': self.picture_path,
            'has_raw': bool(self.raw_data),
            'created_at': to_tashkent(self.created_at),
            'direction': self.direction or 'IN',
        }


# ==================== ATTENDANCE (HIKVISION ASOSIDA) ====================
STAFF_ROLES = ('admin', 'dean', 'teacher', 'edu_dept', 'department_head', 'accounting')
STAFF_STATUS_ON_TIME = 'OnTime'
STAFF_STATUS_LATE = 'Late'
STAFF_STATUS_SERIOUS_LATE = 'Serious Late'
STAFF_STATUS_ABSENT = 'Absent'
STUDENT_STATUS_PRESENT = 'Present'
STUDENT_STATUS_ABSENT = 'Absent'


class StaffAttendanceDaily(db.Model):
    """Xodimlarning kunlik davomati – Hikvision face log asosida hisoblanadi."""
    __tablename__ = 'staff_attendance_daily'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    staff_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False, index=True)
    date = db.Column(db.Date, nullable=False, index=True)
    first_entry = db.Column(db.DateTime, nullable=True)   # birinchi IN
    last_exit = db.Column(db.DateTime, nullable=True)     # oxirgi OUT
    late_minutes = db.Column(db.Integer, default=0)       # 08:30 dan keyin kechikish
    work_duration = db.Column(db.Integer, nullable=True)  # daqiqalarda
    status = db.Column(db.String(20), nullable=False, default=STAFF_STATUS_ABSENT)
    kpi_score = db.Column(db.Integer, default=-2)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    staff = db.relationship('User', backref=db.backref('staff_attendance_daily', lazy='dynamic'))
    __table_args__ = (db.UniqueConstraint('staff_id', 'date', name='uq_staff_attendance_staff_date'),)


class StudentAttendanceDaily(db.Model):
    """Talabalar uchun kunlik davomat – kamida 1 ta IN bo‘lsa Present."""
    __tablename__ = 'student_attendance_daily'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    student_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False, index=True)
    date = db.Column(db.Date, nullable=False, index=True)
    first_entry = db.Column(db.DateTime, nullable=True)
    last_exit = db.Column(db.DateTime, nullable=True)
    status = db.Column(db.String(20), nullable=False, default=STUDENT_STATUS_ABSENT)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    student = db.relationship('User', backref=db.backref('student_attendance_daily', lazy='dynamic'))
    __table_args__ = (db.UniqueConstraint('student_id', 'date', name='uq_student_attendance_student_date'),)
